...
export default {
  ...
  data() {
    return {
      ...
      data:[],
      breadcrumb:[{icon:'home',text:'首页',path:'/stock/workspace'},{icon:'user',text:'律所管理',path:'/stock/workspace'},{icon:'form',text:'律所管理',path:''}],
  ...
  methods: {
      ...
      async execApply(){
          const { $router } = this;
          $router.push(`/stock/firm/firmapply?type=1&tname=律所录入&apply=new`);
      },
      ...
      async execPatch(elem){
          const { $router } = this;
          $router.push(`/stock/firm/firmapply?id=${elem.id}&type=1&tname=律所修改&apply=edit`);
      },

      // 律所查看申请
      async execView(elem){
          const { $router } = this;
          $router.push(`/stock/firm/firmview?id=${elem.id}&type=1&tname=律所详情&apply=view`);
      },
