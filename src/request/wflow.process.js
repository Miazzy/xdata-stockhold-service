...
export async function handleApproveWF(curRow = '', fixedWFlow = '', data = [], tasktype = 'wait') {
    ...
    if (rights.length > 1 && curRow.business_code != "000000000") {
        ...
        if (firstAuditor == "") {
            ...
            try {
                receiveURL = encodeURIComponent(`${window.requestAPIConfig.vuechatdomain}/#/stock/case/legalview?id=${bussinessNode.id}&pid=&tname=bs_reward_apply&panename=myrewardlist&typename=hr_admin_ids&bpm_status=4&proponents=${bussinessNode.hr_admin_ids}&role=hr`);
            ...
            try {
                receiveURL = encodeURIComponent(`${window.requestAPIConfig.vuechatdomain}/#/stock/case/legalview?id=${bussinessCodeID}&pid=&tname=bs_reward_apply&panename=mytodolist&typename=wflow_done&bpm_status=4&proponents=${bussinessNode.create_by}`);
            ...
            try {
                //查询奖惩明细数据
                const list = await query.queryTableDataByPid('bs_reward_items', bussinessCodeID); //查询股权管理明细数据
            ...
            if (vflag == 0) {
                ...
                await handleNotifyHR(firstAuditor, curRow["proponents"], '', encodeURIComponent(`${window.requestAPIConfig.vuechatdomain}/#/stock/case/legalview?id=${curRow.business_data_id}&pid=${pnode.id}&tname=bs_reward_apply&panename=mytodolist&typename=wflow_todo&bpm_status=${bpmStatus.bpm_status}&proponents=${curRow["proponents"]}`));
                ...
                //发送审批流程通知，通知流程下一位审批人，点击审批详情，处理用户提交的股权管理流程审批通知。
                try {
                    const receiveURL = encodeURIComponent(`${window.requestAPIConfig.vuechatdomain}/#/stock/case/legalview?id=${bussinessCodeID}&pid=&tname=bs_reward_apply&panename=mytodolist&typename=wflow_todo&bpm_status=2&proponents=${firstAuditor}`);
